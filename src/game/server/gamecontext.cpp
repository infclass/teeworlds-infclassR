...
{	
	...
	for(; c<sizeof(aNameFound)-1; c++)
	{
		if(pStr[c] == ' ' || pStr[c] == 0)
		{
			...
			else if(str_comp(aNameFound, "!fking") == 0 && m_apPlayers[ClientID] && m_apPlayers[ClientID]->GetCharacter())
			{
				CheckClass = PLAYERCLASS_FKING;
				str_copy(aChatTitle, "fking", sizeof(aChatTitle));
			}
...
bool CGameContext::ConHelp(IConsole::IResult *pResult)
{
	...
	else if(str_comp_nocase(pHelpPage, "soldier") == 0)
	{
		...
		Buffer.append("\n\n");
		Server()->Localization()->Format_L(Buffer, pLanguage, _("Use the hammer to place the bomb and explode it multiple times."), NULL);
	}
